...
export class HistogramPanel extends Autodesk.Viewing.UI.DockingPanel {
    ...
    async setModel(model) {
        ...
        const filteredChartNames = propertyNames.filter(item => CHARTS_VALUE.includes(item));
        console.log("test ", filteredChartNames);
        this.select.innerHTML = filteredChartNames.map(prop => `<option value="${prop}">${prop}</option>`).join('\n');
