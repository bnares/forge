...
class HistogramExtension extends BaseExtension {
    ...
    onToolbarCreated() {
        this._barChartPanel = new HistogramPanel(this, 'dashboard-barchart-panel', 'Bar Chart', { x: 10, y: 10, chartType: 'bar' });
        this._pieChartPanel = new HistogramPanel(this, 'dashboard-piechart-panel', 'Pie Chart', { x: 10, y: 420, chartType: 'doughnut' });
    ...
    async findPropertyValueOccurrences(model, propertyName) {
        ...
        return new Promise(function (resolve, reject) {
            model.getBulkProperties(dbids, { propFilter: [propertyName] }, function (results) {
                ...
                console.log("HistogramExtension MY TEST result: ", results)
                const CHARTS_VALUE = ["Type Name", "Structural Material", "Level", "WB_Scheduled_Level"];
                for (const result of results) {
                    if (result.properties.length > 0) {
                        //console.log("PROPERTIES VALUE ", result.properties[0].displayName.toLowerCase());
                        if (CHARTS_VALUE.includes(result.properties[0].displayName)) {
                            //console.log("inseide IF you are ", result.properties[0].displayName.toLowerCase());
